...
test('push service worker and web app manifest assets exist', () => {
  ...
  assert.match(styles, /\.caldev-toolbar/);
  assert.doesNotMatch(styles, /\.month-calendar-shell/);
  ...
  assert.doesNotMatch(siteContent, /initMonthCalendars/);
  assert.doesNotMatch(siteContent, /\/api\/calendar-events/);
  assert.doesNotMatch(siteContent, /data-month-calendar/);
  ...
  assert.doesNotMatch(workerSrc, /\/api\/calendar-events/);

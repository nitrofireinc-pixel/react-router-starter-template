...
test('parseInstagramGallerySyncState keeps pending/posted/dismissed buckets', () => {
  const empty = parseInstagramGallerySyncState(null);
  assert.deepEqual(empty, { pending: {}, posted: {}, dismissed: {} });
  const parsed = parseInstagramGallerySyncState(JSON.stringify({
    pending: { '12': { reason: 'publish_failed', error: 'timeout' } },
    posted: { '9': { post_id: 'abc' } },
  }));
  assert.equal(parsed.pending['12'].reason, 'publish_failed');
  assert.equal(parsed.posted['9'].post_id, 'abc');
  assert.deepEqual(parsed.dismissed, {});
});

test('Instagram gallery retry UI is wired into Social Media ADMIN_HTML', () => {
  const root = join(dirname(fileURLToPath(import.meta.url)), '..');
  const worker = readFileSync(join(root, 'worker/src/worker.mjs'), 'utf8');
  assert.match(worker, /zernio-instagram-gallery-queue/);
  assert.match(worker, /zernio-instagram-gallery-retry-all/);
  assert.match(worker, /\/api\/admin\/zernio\/instagram\/gallery\/retry/);
  const admin = readFileSync(join(root, 'admin.js'), 'utf8');
  assert.match(admin, /retryAllZernioInstagramGallery/);
  assert.match(admin, /loadZernioInstagramGalleryQueue/);
});

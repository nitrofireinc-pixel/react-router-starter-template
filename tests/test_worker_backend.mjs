...
test('subscribe deep link and print-only QR assets are wired', () => {
  ...
  assert.match(readFileSync(join(root, 'assets/downloads/efhsband-business-cards.pdf')).slice(0, 5).toString('utf8'), /^%PDF-/);
  const qrPage = readFileSync(join(root, 'qr.html'), 'utf8');
  assert.match(qrPage, /site-home-qr\.png/);
  assert.match(qrPage, /Check Us Out!/);
  assert.match(qrPage, /business-cards\.html/);
  assert.match(qrPage, /Sponsor!/);
  assert.match(qrPage, /Donate!/);
  assert.match(qrPage, /Subscribe!/);
  const cardsPage = readFileSync(join(root, 'business-cards.html'), 'utf8');
  assert.match(cardsPage, /Check Us Out!/);
  assert.match(cardsPage, /efhsband-business-cards\.pdf/);
  assert.match(cardsPage, /site-home-qr\.png/);
  assert.match(readFileSync(join(root, 'worker/scripts/sync-public.mjs'), 'utf8'), /business-cards\.html/);
...
test('admin Manage menu includes Resources parent for Business Cards and QR Codes', () => {
  ...
  assert.match(workerSrc, /data-tab="badge-creator">Badge Creator</);
  assert.match(workerSrc, /id="tab-badge-creator"/);
  const adminJs = readFileSync(join(root, 'admin.js'), 'utf8');
  ...
  assert.match(adminJs, /canAccessBadgeCreator/);
  assert.match(adminJs, /'badge-creator': canAccessBadgeCreator\(\)/);
});

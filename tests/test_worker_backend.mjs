...
test('extractEmailAddress and STOP detection', () => {
  ...
  assert.equal(isDeliverableEmailListAddress('parent@example.com'), false);
  assert.equal(isDeliverableEmailListAddress('dns-check@example.com'), false);
  assert.equal(isDeliverableEmailListAddress('test-list+agent@example.com'), false);
  assert.equal(isDeliverableEmailListAddress('nitrofireinc@gmail.com'), true);
  assert.equal(isDeliverableEmailListAddress('family@efhsband.org'), true);
  assert.equal(emailListFromAddress({}), 'no-reply@efhsband.org');
  assert.equal(emailListFromAddress({ CONTACT_FROM_EMAIL: 'no-reply@efhsband.org' }), 'no-reply@efhsband.org');
  assert.equal(emailListFromAddress({ CONTACT_FROM_EMAIL: 'someone@other.org' }), 'no-reply@efhsband.org');
  ...
  assert.match(workerSrc, /isDeliverableEmailListAddress\(row\?\.email\)/);
  assert.match(workerSrc, /emailListFromAddress\(env\)/);
  assert.match(workerSrc, /\/api\/admin\/caldev\/notify-finished/);
});
...
test('ensureCalendarIcalSubscribe adds Apple Calendar button for iPhone iCal', () => {
  const withEmail = `<section class="content email-list-signup" data-email-list-signup data-email-list-topics="calendar,fundraising">
  <div class="wrap email-list-signup-inner">
    <div class="email-list-signup-copy"><h2>Email calendar updates</h2><p>Get calendar changes by email.</p></div>

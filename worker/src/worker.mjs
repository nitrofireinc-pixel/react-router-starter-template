...
async function handleZernioFacebookConnect(request, env) {
  try {
    ...
    if (!(await zernioConfigured(env))) {
      return htmlResponse('<!doctype html><title>Zernio not configured</title><p>Zernio API key is missing at runtime. Open <a href="/admin?tab=social">Social / Facebook</a> as Super Admin and paste the key, or re-save <code>ZERNIO_API_KEY</code> in Cloudflare Pages secrets and redeploy.</p><p><a href="/admin?tab=social">Back to Social / Facebook</a></p>', 503);
    }
...
<script src="/script.js?v=${ASSET_VERSION}"></script><script src="/image-upload.js?v=${ASSET_VERSION}"></script><script src="/site-content.js?v=${ASSET_VERSION}"></script>
</body></html>`;
}
...
async function serveStaticOrCms(request, env, url) {
  ...
  if (['admin.js', 'site-content.js', 'image-upload.js', 'script.js', 'styles.css', 'push-sw.js', 'manifest.webmanifest'].includes(assetName)) {
  ...
  if (assetUrl.pathname === '/assets/downloads/EFHS-Band-Calendar.apk' && assetResponse.ok) {
    const headers = new Headers(assetResponse.headers);
    headers.set('content-type', 'application/vnd.android.package-archive');
    headers.set('content-disposition', 'attachment; filename="EFHS-Band-Calendar.apk"');
    headers.set('cache-control', 'public, max-age=300');
    return new Response(assetResponse.body, { status: assetResponse.status, statusText: assetResponse.statusText, headers });
  }
...
<section id="tab-fundraising-body" class="cms-panel fundraising-body-panel" hidden><div class="panel-head"><div><p class="kicker">Support</p><h1>Fundraising</h1><p>Requires the Fundraising page permission. Click the red Edit button to open the editor.</p></div><div class="panel-actions"><button class="btn primary" type="button" id="edit-fundraising-body">Edit</button></div></div><div class="admin-card stack fundraising-body-card"><h2>Current content</h2><p class="muted">Open the editor to update campaign copy and insert photos. The Direct Support donate card stays on the public page automatically.</p><div id="fundraising-body-preview" class="cms-content fundraising-body-preview"></div><p class="status" id="fundraising-body-panel-status"></p></div>
<div id="fundraising-editor-modal" class="minutes-frame-modal fundraising-editor-modal" hidden>
  <button type="button" class="minutes-frame-backdrop" data-fundraising-editor-dismiss aria-label="Close fundraising editor"></button>
  <div class="minutes-editor-dialog fundraising-editor-dialog admin-card stack" role="dialog" aria-modal="true" aria-labelledby="fundraising-editor-title">
    <div class="minutes-editor-head">
      <div>
        ...
        <h2 id="fundraising-editor-title">Edit Fundraising</h2>
      </div>
    </div>
    <form id="fundraising-body-form" class="stack minutes-editor-form fundraising-editor-form" novalidate>
      <label class="full form-rich-label"><span>Fundraising content</span>${FUNDRAISING_BODY_RICH_TOOLBAR}<div class="form-rich-editor cms-edit-rich fundraising-body-editor" contenteditable="true" role="textbox" spellcheck="true" data-rich-input="body_html" data-rich-mode="block" data-placeholder="Enter fundraising content and photos…" aria-label="Fundraising content"></div><input type="hidden" name="body_html"></label>
      <div class="panel-actions minutes-form-actions">
...
<script src="/image-upload.js?v=${ASSET_VERSION}"></script><script src="/admin.js?v=${ASSET_VERSION}"></script></body></html>`;

...
const ASSET_VERSION = 'admin-cms-20260802-54';
...
export function normalizeHeaderNavItems(items = []) {
  ...
  return list
    .map((item, index) => {
      ...
      seen.add(slug);
      const title = String(item?.title || '').replace(/\s*\|\s*East Forsyth Band$/i, '').trim();
      const activeRaw = item?.active;
      const active = activeRaw === false || activeRaw === 0 || activeRaw === '0' ? 0 : 1;
      const order = Number(item?.nav_order);
      return {
        slug,
        title,
        nav_order: Number.isFinite(order) ? order : index,
        active,
      };
    })
    .filter(Boolean)
    .sort((a, b) => a.nav_order - b.nav_order || a.slug.localeCompare(b.slug))
    .map((item, index) => ({ ...item, nav_order: index }));
}

function canManageHeaderNav(user) {
  return hasPermission(user, 'pages') || canEditPage(user, 'home');
}

async function getHeaderNavItems(env) {
  const pages = await getPages(env, true);
  return pages
    .map((page) => ({
      slug: page.slug,
      title: String(page.title || '').replace(/\s*\|\s*East Forsyth Band$/i, '').trim() || page.slug,
      path: page.path,
...
async function handleApi(request, env, url) {
  ...
  if (url.pathname === '/api/admin/pages/nav' && request.method === 'GET') {
    const auth = await requireLogin(request, env);
    if (auth.response) return auth.response;
    if (!canManageHeaderNav(auth.user)) return jsonResponse({ detail: 'Permission required: page:home' }, 403);
    return jsonResponse(await getHeaderNavItems(env));
  }
  if (url.pathname === '/api/admin/pages/nav' && request.method === 'PUT') {
    const auth = await requireLogin(request, env);
    if (auth.response) return auth.response;
    if (!canManageHeaderNav(auth.user)) return jsonResponse({ detail: 'Permission required: page:home' }, 403);
    const payload = await request.json();
    const items = normalizeHeaderNavItems(payload.items || payload);
    if (!items.length) return jsonResponse({ detail: 'At least one navigation item is required' }, 422);
    for (const item of items) {
      const existing = await getPageBySlug(env, item.slug, true);
      if (!existing) continue;
      const title = item.title || String(existing.title || '').replace(/\s*\|\s*East Forsyth Band$/i, '').trim() || existing.slug;
      const active = existing.is_home || existing.slug === 'home' ? 1 : item.active;
      await env.DB.prepare('UPDATE cms_pages SET title = ?, nav_order = ?, active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
        .bind(title, item.nav_order, active, existing.id)
        .run();
    }
    return jsonResponse(await getHeaderNavItems(env));
  }

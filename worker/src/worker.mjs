...
const ASSET_VERSION = 'admin-cms-20260803-23';
...
async function storeImageUpload(env, file, altText = '', caption = '', sortOrder = 0) {
  ...
  const cleanCaption = captionRaw
    ...
    : '';
  const result = await env.DB.prepare('INSERT INTO photos (filename, original_name, alt_text, caption, sort_order, content_type, data_base64) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(filename, originalName, cleanAlt, cleanCaption, Number(sortOrder || 0), file.type || 'application/octet-stream', dataBase64).run();
  return { id: result.meta.last_row_id, filename, original_name: originalName, alt_text: cleanAlt, caption: cleanCaption, sort_order: Number(sortOrder || 0), url: `/uploads/${encodeURIComponent(filename)}` };
}
...
<section id="tab-dashboard" class="cms-panel dashboard-panel"><div class="panel-head"><div><p class="kicker">Administration</p><h1 id="dashboard-welcome">Welcome back</h1><p>Changes save to the shared CMS database and publish to the public East Forsyth Band website.</p></div><a class="btn primary" href="/" target="_blank" rel="noreferrer">View Site</a></div><div id="dashboard-cards" class="dashboard-cards"></div><form id="password-form" class="admin-card stack dashboard-password-card"><h2>Change your password</h2><p class="muted">Enter your current password, then choose a new one (at least 8 characters).</p><label>Current password<input name="current_password" type="password" required autocomplete="current-password"></label><label>New password<input name="new_password" type="password" required minlength="8" autocomplete="new-password"></label><label>Confirm new password<input name="confirm_password" type="password" required minlength="8" autocomplete="new-password"></label><button class="btn primary" type="submit">Update password</button><p class="status" id="password-status"></p></form></section>

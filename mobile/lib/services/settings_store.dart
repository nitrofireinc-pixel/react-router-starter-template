...
class SettingsStore {
  ...
  static const defaultSourceUrl = 'https://efhsband.org/api/caldev/events';
  static const defaultApiBaseUrl = 'https://efhsband.org';
  static const legacySourceUrls = {
    'https://efhsband.org/api/events',
    'https://efhsband.org/api/calendar-events',
  };
  ...
  static Future<SettingsStore> open() async {
    ...
    await store.migrateLegacyDefaults();
    return store;
  }

  Future<void> migrateLegacyDefaults() async {
    final current = (_prefs.getString(_sourceKey) ?? '').trim();
    if (current.isEmpty || legacySourceUrls.contains(current)) {
      await _prefs.setString(_sourceKey, defaultSourceUrl);
    }
  }

  String get sourceUrl {
    final value = (_prefs.getString(_sourceKey) ?? '').trim();
    if (value.isEmpty || legacySourceUrls.contains(value)) {
